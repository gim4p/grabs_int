...
function step2_gen_method4_method5(fname_out_int,folder_out,fname_out_method4,fname_out_method5)
...
gsc_mat = ones(size(grabsint.claytongridinterpolation.psd_all2D));
for n = 1 : length(grabsint.gsc_sievers_recon);
    gsc_mat(:,n) = gsc_mat(:,n)*grabsint.gsc_sievers_recon(n);
end

method4_mean = nansum(gsc_mat.*grabsint.claytongridinterpolation.psd_all2D,2);

clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.X_grid;
method4_X = reshape(mat_in, [size(mat_in,1), size(mat_in,2)*size(mat_in,3) ] )';
clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.Y_grid;
method4_Y = reshape(mat_in, [size(mat_in,1), size(mat_in,2)*size(mat_in,3) ] )';


if calculate_structs_new == 1
    
    psd_all2D = [grabsint.claytongridinterpolation.psd_all2D];
    gsc = [grabsint.gsc_sievers_recon];
    
    startstop = round(linspace(0,length(grabsint.claytongridinterpolation.psd_all2D),12));
    ...
    for mm = 1 : length(startstop)-1
        ...
        [method4_median, method4_D10, method4_D90, ~]=medianstat(psd_all2D(startn:stopn,:),gsc);

        [method4_mode_size_1,method4_mode_size_2,method4_mode_size_3,method4_modestatus]=modestat(psd_all2D(startn:stopn,:),gsc);
        pause(.1)
    ...
    psd_3dtmp = grabsint.claytongridinterpolation.psd_all3D;
    psd_3d = psd_3dtmp;
    psd_2dtmp = grabsint.claytongridinterpolation.psd_all2D;
    psd_2d = psd_2dtmp;
    
    method4.X_grid = grabsint.claytongridinterpolation.X_grid;
    method4.Y_grid = grabsint.claytongridinterpolation.Y_grid;
...
clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.stat_mean;
method5_mean = reshape(mat_in, [size(mat_in,1), size(mat_in,2)*size(mat_in,3) ] )';
clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.stat_median;
method5_median = reshape(mat_in, [size(mat_in,1), size(mat_in,2)*size(mat_in,3) ] )';
% clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.stat_D10;
% method5_D10 = reshape(mat_in, [size(mat_in,1), size(mat_in,2)*size(mat_in,3) ] )';
% clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.stat_D90;
% method5_D90 = reshape(mat_in, [size(mat_in,1), size(mat_in,2)*size(mat_in,3) ] )';
clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.stat_mode_1;
method5_mode1 = reshape(mat_in, [size(mat_in,1), size(mat_in,2)*size(mat_in,3) ] )';

clear mat_in; mat_in(1,:,:) = grabsint.claytongridinterpolation.stat_modestatus;
...
psd_3dtmp = grabsint.claytongridinterpolation.psd_recon_all3D;
psd_3d = psd_3dtmp;
psd_2dtmp = grabsint.claytongridinterpolation.psd_recon_all2D;
psd_2d = psd_2dtmp;

method5.X_grid = grabsint.claytongridinterpolation.X_grid;
method5.Y_grid = grabsint.claytongridinterpolation.Y_grid;
...
%% gistable
make_gis_table_method4(method4,grabsint,fname_out_method4,folder_out)
cd(functionpath)
make_gis_table_method5(method5,grabsint,fname_out_method5,folder_out)
cd(functionpath)
